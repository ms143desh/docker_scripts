...
docker exec -it mongostarget1 bash -c "echo 'sh.addShard(\"mongotargetrs1/mongotargetrs1n1:27018,mongotargetrs1n2:27018,mongotargetrs1n3:27018\")' | mongo"
sleep 10
...
docker exec -it mongostarget1 bash -c "echo 'rs.status()' | mongo"
sleep 10

echo "MongoShell - run command - db.adminCommand({listShards:1})"
docker exec -it mongostarget1 bash -c "echo 'db.adminCommand({listShards:1})' | mongo"
sleep 10

echo "MongoShell - run command - db.printShardingStatus()"
docker exec -it mongostarget1 bash -c "echo 'db.printShardingStatus()' | mongo"
sleep 10

echo "MongoShell - run command - db.hostInfo()"
docker exec -it mongostarget1 bash -c "echo 'db.hostInfo()' | mongo"
sleep 10

echo "MongoShell - run command - db.stats()"
docker exec -it mongostarget1 bash -c "echo 'db.stats()' | mongo"
sleep 10

echo "MongoShell - run command - db.serverStatus()"
docker exec -it mongostarget1 bash -c "echo 'db.serverStatus()' | mongo"
